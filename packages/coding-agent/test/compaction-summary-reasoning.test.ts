...
describe("generateSummary reasoning options", () => {
	...
	it("appends instructions to a cache-friendly source context", async () => {
		const sourceContext: Context = {
			...
			messages: [
				{
					role: "user",
					content: "Previous summary and original request",
					timestamp: 1,
				},
			],
			tools: [],
		...
		expect(streamSimpleMock.mock.calls[0][2]?.onPayload).toBe(onPayload);
		expect(streamSimpleMock.mock.calls[0][2]?.onResponse).toBe(onResponse);
	...
	it("falls back to standalone summarization for an oversized split-turn source", async () => {
		...
		await compact(
			...
			undefined,
			{
				...
				requestOptions: { sessionId: "routing-session" },
			},
		);
	...
	it("rejects tool calls from conversation summaries", async () => {
		streamSimpleMock.mockReturnValueOnce(fakeSummaryStream(mockToolCallResponse));
	...
	it("does not set Anthropic refusal fallback for models without allowed fallback targets", async () => {
		...
		expect(streamSimpleMock).toHaveBeenCalledTimes(1);
		expect(streamSimpleMock.mock.calls[0][2]).not.toHaveProperty("refusalFallbacks");
	});
