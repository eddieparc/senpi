...
describe("--session-id", () => {
	...
	it("creates missing IDs and reopens existing IDs in process", async () => {
		...
		const noSession = await createSessionManager(
			args({ sessionId: "ephemeral-id", noSession: true }),
			projectDir,
			sessionDir,
			settingsManager,
			"print",
		);
		expect(noSession.getSessionId()).toBe("ephemeral-id");
		expect(noSession.getSessionFile()).toBeUndefined();

		const listModels = await createSessionManager(
			args({ sessionId: "read-only-models", listModels: true }),
			projectDir,
			sessionDir,
			settingsManager,
			"print",
		);
		expect(listModels.getSessionId()).toBe("read-only-models");
		expect(listModels.getSessionFile()).toBeUndefined();
